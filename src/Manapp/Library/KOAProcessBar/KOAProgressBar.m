...
- (void)initializeProgressBarWithColor:(UIColor *) color {
	_animator = nil;
	self.progressOffset = 0.0;
	self.stripeWidth = 10.0;
	self.inset = 2.0;
	...
	self.progressBarColorBackgroundGlow = [UIColor colorWithRed:17.0/255.0 green:20.0/255.0 blue:23.0/255.0 alpha:1.0];
	self.stripeColor = [UIColor colorWithRed:101.0/255.0 green:151.0/255.0 blue:120.0/255.0 alpha:0.9];
	self.lighterProgressColor = color;
	self.darkerProgressColor = color;
	self.lighterStripeColor = color;
	self.darkerStripeColor = color;
	self.displayedWhenStopped = YES;
	self.timerInterval = 0.1;
	self.progressValue = 0.01;
...
- (void)awakeFromNib
{
    ...
    [self initializeProgressBarWithColor:RGB(248, 78, 39)];
...
-(void) tap:(id)sender{
    [self.delegate didTouchKOAProgressBar:self];
}
...
-(void)drawStripesInBounds:(CGRect)frame {
	koaGradient *gradient = [[[koaGradient alloc] initWithStartingColor:self.lighterStripeColor endingColor:self.darkerStripeColor] autorelease];
    UIBezierPath* allStripes = [[[UIBezierPath alloc] init] autorelease];
...
-(void)drawProgressWithBounds:(CGRect)frame {
    ...
    koaGradient *gradient = [[[koaGradient alloc] initWithStartingColor:self.lighterProgressColor endingColor:self.darkerProgressColor] autorelease];
