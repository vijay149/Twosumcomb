...
#pragma mark -
#pragma mark truncate the string when it reach a certain length
-(NSString *) truncateAtLength:(NSInteger) length{
    if(self.length <= length){
        return self;
    }
    else{
        NSString *substring = [self substringWithRange:NSMakeRange(0, length)];
